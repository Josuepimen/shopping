        ...
        {data.map((dessert, index) => (
          <div key={index} className='rounded-lg p-4 shadow-lg w-full max-w-xs'>
            ...
            <button 
              ...
              className='mt-4 rounded-3xl border border-amber-700 p-3 flex justify-center font-RedHat font-semibold text-orange-950 w-44 hover:bg-zinc-200 transition duration-200'
            >
      ...
      <div className=' bottom-0 left-0 right-0 bg-white shadow-lg p-4'>
        ...
        {totalItems === 0 ? (
          <div className='flex flex-col items-center'>
            <img src="../public/assets/images/illustration-empty-cart.svg" alt="No items in cart" className='w-32 h-32' />
            <p className='text-yellow-950 font-RedHat font-semibold'>Your added items will appear here</p>
          </div>
        ) : (
          cart.map((item, index) => (
        ...
        <button onClick={handleStartNewOrder} className='mt-2 rounded-3xl border border-red-700 p-3 flex justify-center font-RedHat font-semibold text-red-950 w-44 hover:bg-red-200 transition duration-200'>
          Start New Order
        </button>
      </div>
      ...
      {isConfirmationModalOpen && (
        <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='bg-white p-6 rounded-lg shadow-lg'>
            ...
            <button onClick={() => setIsConfirmationModalOpen(false)} className='mt-4 rounded-3xl border border-blue-700 p-2 text-blue-700 hover:bg-blue-200'>
              Close
            </button>
          </div>
        </div>
